...
class TaskNotificationManager {
  ...
  Future<void> _startTaskListener() async {
    ...
    if (currentUser == null) {
      print(
        'TaskNotificationManager: No authenticated user, skipping task listener',
      );
      return;
    }

    print(
      'TaskNotificationManager: Starting task listener for user: ${currentUser.uid}',
    );
  ...
  Future<void> _handleTaskChanges(QuerySnapshot snapshot) async {
    print(
      'TaskNotificationManager: Processing ${snapshot.docs.length} active tasks',
    );
  ...
  Future<void> _scheduleTaskNotifications(Task task) async {
    try {
      ...
      if (!task.isReminder || task.isCompleted || task.isArchived) {
        print(
          'TaskNotificationManager: Skipping notification for task ${task.id} (no reminder or completed/archived)',
        );
        return;
      }
      ...
      if (dueDate.isBefore(now)) {
        print(
          'TaskNotificationManager: Task ${task.id} due date is in the past, skipping notification',
        );
        return;
      }

      print(
        'TaskNotificationManager: Scheduling notifications for task: ${task.title}',
      );
      ...
      for (DateTime notificationTime in notificationTimes) {
        if (notificationTime.isAfter(
          ...
          // Allow 1 minute buffer for due time
          ...
          if (notificationTime.isAtSameMomentAs(dueDate) ||
              notificationTime.difference(dueDate).abs() <
            ...
            titlePrefix =
                '${_getPriorityEmoji(task.priority)} Task Deadline $urgencyText';
          }
          ...
          print(
            '  ✓ Scheduled notification $notificationIndex for ${_formatDueDate(notificationTime)} ($urgencyText)',
          );
        }
        notificationIndex++;
      }
      ...
      print(
        'TaskNotificationManager: Error scheduling notifications for task ${task.id}: $e',
      );
    }
  }
  ...
  Future<void> _cancelTaskNotifications(Task task) async {
    try {
      ...
      print(
        'TaskNotificationManager: Cancelled notifications for task: ${task.title}',
      );
    } catch (e) {
      print(
        'TaskNotificationManager: Error cancelling notifications for task ${task.id}: $e',
      );
    }
  }
  ...
  void _startDeadlineChecker() {
    ...
    print(
      'TaskNotificationManager: Started periodic deadline checker (every 15 minutes)',
    );
  }
  ...
  void _startRealTimeChecker() {
    ...
    print(
      'TaskNotificationManager: Started real-time checker (every 1 minute)',
    );
  }
  ...
  Future<void> _checkTasksDueRightNow() async {
    try {
      ...
      final QuerySnapshot dueNowTasksSnapshot =
          await _firestore
              .collection('tasks')
              .where('userId', isEqualTo: currentUser.uid)
              .where('isCompleted', isEqualTo: false)
              ...
              .where(
                'dueDate',
                isGreaterThanOrEqualTo: Timestamp.fromDate(currentMinute),
              )
      ...
      if (dueNowTasksSnapshot.docs.isNotEmpty) {
        print(
          'TaskNotificationManager: Found ${dueNowTasksSnapshot.docs.length} tasks due right now',
        );
        ...
        for (QueryDocumentSnapshot doc in dueNowTasksSnapshot.docs) {
          ...
          print(
            'TaskNotificationManager: Sent immediate "DUE NOW" notification for: ${task.title}',
          );
        }
      }
    } catch (e) {
  ...
  Future<void> _checkRecentlyDueTasksAndNotify() async {
    try {
      ...
      final now = DateTime.now();
      final recentPast = now.subtract(
        const Duration(minutes: 30),
      ); // Check last 30 minutes

      final QuerySnapshot recentlyDueTasksSnapshot =
          await _firestore
              .collection('tasks')
              .where('userId', isEqualTo: currentUser.uid)
              .where('isCompleted', isEqualTo: false)
              ...
              .where(
                'dueDate',
                isGreaterThanOrEqualTo: Timestamp.fromDate(recentPast),
              )
      ...
      if (recentlyDueTasksSnapshot.docs.isNotEmpty) {
        print(
          'TaskNotificationManager: Found ${recentlyDueTasksSnapshot.docs.length} recently due tasks',
        );
        ...
        for (QueryDocumentSnapshot doc in recentlyDueTasksSnapshot.docs) {
          ...
          if (timeSinceDue.inMinutes <= 30 && timeSinceDue.inMinutes >= 0) {
            ...
            await _notificationService.showImmediateNotification(
              title: '🔥 TASK DUE NOW: ${task.title}',
              body:
                  'This task just became due!\n📅 Due: ${_formatDueDate(task.dueDate)}${task.description != null ? '\n📝 ${task.description}' : ''}',
              payload: 'due_now_task:${task.id}',
            );

            print(
              'TaskNotificationManager: Sent "due now" notification for task: ${task.title}',
            );
          }
        }
      }
  ...
  Future<void> _checkOverdueTasksAndNotify() async {
    try {
      ...
      final now = DateTime.now();
      final QuerySnapshot overdueTasksSnapshot =
          await _firestore
              .collection('tasks')
              .where('userId', isEqualTo: currentUser.uid)
              .where('isCompleted', isEqualTo: false)
      ...
      if (overdueTasksSnapshot.docs.isNotEmpty) {
        print(
          'TaskNotificationManager: Found ${overdueTasksSnapshot.docs.length} overdue tasks',
        );
        ...
        for (QueryDocumentSnapshot doc in overdueTasksSnapshot.docs) {
          ...
          await _notificationService.showImmediateNotification(
            ...
            body:
                'This task was due ${_formatOverdueDuration(overdueDuration)} ago.\n📅 Was due: ${_formatDueDate(task.dueDate)}${task.description != null ? '\n📝 ${task.description}' : ''}',
